...
-m lddt               기능 8: per-column LDDT 색상 모드 (--foldmason JSON 사용 시; FASTA는 -m conservation)
```
...
- `--foldmason`: FoldMason MSA 출력(`result.json` 또는 `result_aa.fa`)에서 공통 정렬 잔기를 추출 → Kabsch SVD로 superposition
...
| `src/structure/Parameters.hpp` | 수정 | `--foldmason`, `-m lddt` 인수 추가 |
