...
8가지 신규 기능을 순서대로 구현한다. 각 기능은 독립적으로 동작 가능하도록 설계하되, 공통 인프라(Atom 필드 확장, Palette 확장)는 먼저 처리한다.
...
5a. **기능 4** - UTMatrix 정렬 구조 색상 표시(-ut)   ✅ **완료**
...
85-94: ❌ 미사용 (FoldMason per-column LDDT용으로 예약되었으나 -m lddt 제거로 미구현)
```
...
❌ -m lddt            기능 8에서 제거됨 (per-column LDDT 색상 모드 — 불필요한 복잡성으로 삭제)
```
...
**`--db-path`는 선택 인수다.** 지정하면 로컬 DB 디렉토리에서 파일을 우선 탐색하고, 없으면 `~/.cache/structty/pdb/`에서 캐시를 확인 후 자동 다운로드를 시도한다.
...
N/P 키 입력 시 hit을 lazy load 방식으로 처리한다 (BUG-C 참조: 사전 다운로드 없음).
...
# 기능 3: Foldseek hit 탐색 — lazy load (로컬 DB 있을 때, 자동 다운로드 불필요)
structty query.pdb -fs result.m8 --db-path /data/pdb
# 기능 3: Foldseek hit 탐색 — lazy load (--db-path 없으면 자동 다운로드)
structty query.pdb -fs result.m8
