...
--foldmason <file>    기능 8: FoldMason JSON 또는 FASTA MSA 파일 경로
--foldmason-db <dir>  기능 8: entry 구조 파일 디렉토리 (PDB/mmCIF, [ / ] 탐색용)
-m foldmason          기능 8: per-column LDDT 색상 모드
```

기존 `-m` 옵션의 유효값 목록에 위 5가지 추가.
...
FoldMason의 `easy-msa --report-mode 2`가 생성하는 JSON 결과 파일 또는 FASTA MSA 파일을 파싱하여:

1. 쿼리 구조를 per-column LDDT 점수 기준으로 색상 표시 (`-m foldmason`)
2. MSA entries를 `[`/`]` 키로 순회하며 각 entry 구조를 쿼리와 중첩 표시 (`--foldmason-db`)

### 입력 파일 형식

```bash
# JSON (권장): aa, ss(3Di), ca, scores, tree 모두 포함
foldmason easy-msa *.pdb result tmpFolder --report-mode 2
→ result.json

# FASTA: aa 서열만 (LDDT scores 없음, conservation entropy만 계산 가능)
...
- `.json` → JSON 파싱
- `.fa`, `.fasta` → FASTA 파싱 (aa 또는 3Di 파일 모두 허용)
...
public:
    // JSON 파일 파싱 (result.json, --report-mode 2 출력)
    bool load_json(const std::string& path);

    // FASTA MSA 파일 파싱 (result_aa.fa 또는 result_3di.fa)
    ...
    int msa_length() const;                        // 정렬 열 수
    int entry_count() const;

    // 쿼리(entries[ref_idx]) 잔기 → MSA 열 매핑 구축
    // gap이 없는 위치만 등록: aa[col] != '-' → residue_col_map[residue_idx] = col
    std::vector<int> build_query_col_map(int ref_idx = 0) const;

    // 열별 Shannon entropy 계산 (aa 또는 ss 컬럼)
    std::vector<float> compute_column_entropy(bool use_ss = false) const;

private:
    std::vector<FoldMasonEntry> entries;
    std::vector<float> scores;   // per-column LDDT
    float msa_lddt = -1.0f;
    std::string tree;            // Newick (옵션)
    std::string format;          // "json" 또는 "fasta"
};
```

**JSON 파싱 명세 (`result.json` 구조):**
...
- `entries[i].ca`는 **gap이 없는 잔기에만** 좌표가 있음 (gap 위치 좌표 없음)
- `scores` 배열 길이 == `entries[i].aa` 길이 (MSA 열 수)
- `statistics.msaLDDT`가 없으면 -1.0 유지

**FASTA 파싱:** 기존 MSAParser와 동일한 방식. 첫 번째 서열을 쿼리(ref)로 간주.

### per-column LDDT 쿼리 색상 표시 (`-m foldmason`)

**동작 흐름:**

```
1. FoldMasonParser::load_json() 호출
2. build_query_col_map(0) 으로 쿼리 잔기 인덱스 → MSA 열 매핑 생성
   map[residue_0based_idx] = msa_col
3. Protein::apply_foldmason_scores() 호출:
   각 잔기 j에 대해 col = map[j] → atom.conservation_score = scores[col]
   (scores 없는 FASTA 로드 시: compute_column_entropy()로 대체)
4. 색상 적용: assign_colors_to_points()에서
   if (mode == "foldmason") {
       float score = point.conservation_score;
...
- MSA 열에서 쿼리 서열에 gap(`-`)인 열은 해당 잔기가 없음 → 매핑에서 제외
- 쿼리 잔기 수 == entries[0].aa에서 '-'를 제외한 문자 수 == Protein CA atom 수 가 일치해야 함; 불일치 시 경고 출력 후 가능한 범위만 매핑

### MSA Entry 구조 탐색 (`--foldmason-db` + `[`/`]` 키)

**동작 흐름:**

```
프로그램 시작:
1. --foldmason JSON/FASTA 로드
2. entries[0] 을 reference(query)로 간주
3. --foldmason-db 지정 시, entries[1..N-1] 의 구조 파일 상위 10개 백그라운드 탐색
   (다운로드 없음, 로컬 --foldmason-db 디렉토리 탐색만)
4. entries[0] 의 구조는 --foldmason 호출 시 함께 제공된 query.pdb를 사용

]키: 다음 entry (load_foldmason_entry(+1))
[키: 이전 entry (load_foldmason_entry(-1))
```

**`Screen::load_foldmason_entry(int delta)` 함수:**

```
1. current_fm_idx += delta (범위 클램핑, 0 = reference 제외, 1..N-1 탐색)
2. FoldMasonEntry& entry = fm_entries[current_fm_idx]
3. --foldmason-db 에서 구조 파일 탐색:
   {foldmason_db}/{entry.name}.pdb
   {foldmason_db}/{entry.name}.cif
   대소문자 변환 시도
4. 파일 없고 entry.ca가 있으면 Cα-only 모드 (CA 점만 표시, helix/sheet 없음)
5. 파일 없고 ca도 없으면 패널에 "File not found: {entry.name}" 표시 후 return
6. entry.aa 로 reference(entries[0].aa)와 pairwise 정렬 잔기 쌍 추출
   (gap-free 위치 기준 공통 잔기 추출)
7. Kabsch SVD로 entry → reference frame superposition (FoldseekParser와 동일)
8. 재렌더링, 패널 entry 정보 업데이트
```

**entry.name → 파일명 변환:**

FoldMason은 입력 파일명에서 확장자를 제거하여 entry.name을 생성한다:
- `query.pdb` → `query`
- `3A0C-assembly1.cif` → `3A0C-assembly1cif_A` (FoldMason 내부 처리 방식에 따라 다를 수 있음)

따라서 다음 순서로 탐색:
```
{db}/{name}.pdb
{db}/{name}.cif
{db}/{name}_*.pdb  (glob, 파일 하나만 매치 시)
{db}/{name_lowercase}.pdb / .cif
```

### 관련 신규 함수: `Protein::apply_foldmason_scores()`

```cpp
// src/structure/Protein.hpp / Protein.cpp
// FoldMason per-column LDDT 또는 entropy 점수를 잔기별 conservation_score에 주입
// scores: 0-based 잔기 인덱스 기준 점수 벡터
...
`-fs` 없이 `--foldmason`만 사용한 경우 아래 섹션 표시:
...
│ Entries: 15         │
│ MSA LDDT: 0.847     │
│ [Entry탐색 비활성]  │   ← --foldmason-db 미지정 시
└─────────────────────┘
```
...
`-fs`와 동시 사용 시: 두 섹션이 패널에 함께 표시된다 (Foldseek 섹션 위, FoldMason 섹션 아래).
...
int current_fm_idx = 0;         // 현재 표시 중인 entry 인덱스 (0 = reference)
std::string foldmason_db_path;
```
...
```cpp
case ']':
    if (foldmason_parser && foldmason_parser->entry_count() > 1 && !foldmason_db_path.empty())
        load_foldmason_entry(+1);
    break;
case '[':
    if (foldmason_parser && foldmason_parser->entry_count() > 1 && !foldmason_db_path.empty())
        load_foldmason_entry(-1);
...
### FASTA만 로드 시 동작 (`_aa.fa` 또는 `_3di.fa`)

- `scores` 없음 → `-m foldmason` 시 Shannon entropy 사용 (`compute_column_entropy()`)
- entropy 기반 색상은 conservation 모드와 동일한 75-84 그래디언트 사용
- entry 탐색은 동일하게 가능 (`--foldmason-db` 있을 경우)
- Panel에 "MSA LDDT: N/A" 표시

### CLI

```bash
# LDDT 색상 (JSON, 권장)
structty query.pdb --foldmason result.json -m foldmason

# LDDT 색상 + entry 탐색
structty query.pdb --foldmason result.json --foldmason-db /path/to/pdbs -m foldmason

# FASTA (entropy 기반 conservation)
structty query.pdb --foldmason result_aa.fa -m foldmason

# Foldseek hit 탐색과 동시 사용
structty query.pdb -fs result.m8 --foldmason msa.json -m foldmason
```
...
| `src/structure/FoldMasonParser.hpp` | **신규** | FoldMasonEntry 구조체, FoldMasonParser 클래스 선언 |
| `src/structure/FoldMasonParser.cpp` | **신규** | JSON 파싱(entries/scores/tree/statistics), FASTA MSA 파싱, query-col 매핑, Shannon entropy 계산 |
...
| `src/structure/Parameters.hpp` | 수정 | `--foldmason`, `--foldmason-db`, `-m foldmason` 인수 추가 |
| `src/visualization/Screen.hpp` | 수정 | `foldmason_parser`, `current_fm_idx`, `foldmason_db_path` 멤버 추가 |
| `src/visualization/Screen.cpp` | 수정 | `[`/`]` 키 처리, `load_foldmason_entry()`, `assign_colors_to_points()`에 foldmason 모드 추가 |
...
# 기능 8: FoldMason LDDT 색상 (JSON)
structty query.pdb --foldmason result.json -m foldmason

# 기능 8: FoldMason entry 구조 탐색 ([ / ] 키)
structty query.pdb --foldmason result.json --foldmason-db /data/pdbs

# 기능 8: FASTA 파일 (entropy 기반)
structty query.pdb --foldmason result_aa.fa -m foldmason

# 기능 8: Foldseek hit 탐색과 동시 사용 (패널에 두 섹션 표시)
structty query.pdb -fs hits.m8 --foldmason result.json -m foldmason
```
...
| `]` | 다음 FoldMason MSA entry 구조 로드 (기능 8) |
| `[` | 이전 FoldMason MSA entry 구조 로드 (기능 8) |
